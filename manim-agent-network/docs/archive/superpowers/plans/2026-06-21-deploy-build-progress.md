...
- [x] Admin dashboard UI (`frontend/admin.html` + `admin.js`): cost cards, status breakdown bars, jobs + users tables, **role-toggle (user mgmt)**, Clerk-gated, 10s refresh, no chart lib *(needs browser verify when keys land)*
- [x] Role endpoint hardened: 404 on unknown user, self-demotion blocked, `esc()` single-quote defense-in-depth *(Santa admin-review suggestions)*
